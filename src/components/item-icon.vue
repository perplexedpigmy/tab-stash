<!--
  ...
  longer necessary on recent browsers, and it causes a major performance
  regression in the popup view--instead of making the browser decide which image to
  use, we only give it a single `src`.

  What happens is: the browser tries to recalculate the size of the popup on every
  ...
  Unfortunately, this can get quite expensive, because as part of this, the browser
  ...
  because the browser was trying to decide which version of the image to use.
